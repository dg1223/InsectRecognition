...
mr_array, all_FPPI, all_FNPI, all_match, total_matches, total_FPs, total_FNs, MRPI, miss_rates = [], [], [], [], [], [], [], [], []
...
feature_database = []
ct, ct1, total_miss_rate = 0, 0, 0
for t in ov_thresh:                            # for every overlap threshold
    print 'OVERLAP_THRESH     = ', t
##    ct += 1
...
##    all_FPPI.append([])
    for p in p_thresh:
##        ct1 += 1
##        total_matches[ct-1].append([])
##        total_FPs[ct-1].append([])
##        total_FNs[ct-1].append([])
##        all_FPPI[ct-1].append([])    
        for z in range(14,15):        # len(img_list2)
            print img_list2[z]
        ...
        ##feature_database_TestSet_all = np.array(feature_database)
                    ...
                    if shape(bb_dt_ref.get(k1))[0] == 1:                            # if one BBdt in grid
                        if (bb_dt_ref.get(k1)[0][0] == bb_dt_pixels[index_p][0][0][0]).all() == True:       # match 1st array
                            for k, v in bb_gt_ref.items():
                                if (k == matched_gt).any() == True:     # remember to make it empty before looping over the next image
                                    continue
                                else:
    ...
    ##                                        print k
            ...
            # for every images
            total_match += match                # total match for all the images
        ...
        print 'match at p_thresh ', p, '  = ', total_match

        total_FPs.append(total_FP)              # total_FP for every threshold value
##        FPPI = total_FP/len(img_list2)           # False Positives Per Image
