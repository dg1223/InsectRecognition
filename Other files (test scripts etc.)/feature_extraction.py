...
train_path = '/export/mlrg/salavi/Desktop/Project/Original Images/Good Images/Positive Counts/Training Set'

##img_list = glob.glob('/Original Images/Good Images/Positive Counts/Training Set*.jpg')                       # creates a list of all the files with the given format
img_list = [f for f in os.listdir(train_path) if f.find('.jpg') > 0]
img_list = np.sort(np.array(img_list))
first_qrtr,  second_qrtr = 112, 145
feature_database = []
for z in range(np.shape(img_list)[0]):                 # shape(img_list)[0], alternatively, len(...)    
...
##    print z, img_list[z]
    ...
    # decode JSON
    json_data =  open(os.path.join(train_path, img_list[z][:-4]))           # img_list[z][:-4]
    data = json.load(json_data)
    brx, tlx, bry, tly = [], [], [], []
    for x in range(np.shape(data["Image_data"]["boundingboxes"][:])[0]):
...
##           return hist, bins, width, centre                # omit width and centre if unnecessary
        ...
        hist_int, bins_int, width_int, centre_int = create_hist_img(pixels_int, im_int_diff)
        hist_hue, bins_hue, width_hue, centre_hue = create_hist_img(pixels_hue, im_hue_diff)
        hist_sat, bins_sat, width_sat, centre_sat = create_hist_img(pixels_sat, im_sat_diff)
...
##               return hist, bins, width, centre 
            ...
            hist, bins, width, centre = create_hist_obj(pixels, im_int, x_axis, y_axis)
